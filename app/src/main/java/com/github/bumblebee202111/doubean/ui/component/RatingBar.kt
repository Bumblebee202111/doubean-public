...
private fun RatingStar(
    ...
    Canvas(modifier = Modifier.size(size)) {
        ...
        // Draw active portion with proper clipping
...
private fun DrawScope.drawStar(color: Color, size: Float) {
    val center = size / 2
    val radius = size / 2
    val innerRadius = radius * 0.4f  // 40% of total size

    val path = Path().apply {
        moveTo(center, 0f)  // Start at top point
...
private fun PreviewEdgeCases() {
    Column {
        DoubeanRatingBar(rating = 5.5f)  // Clamped to 5.0
        DoubeanRatingBar(rating = 2.7f)  // 2 full, 1 half
        DoubeanRatingBar(rating = 0.4f)  // All empty
    }
}