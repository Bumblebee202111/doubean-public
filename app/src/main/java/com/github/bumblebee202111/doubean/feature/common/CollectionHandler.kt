...
/**
 * A reusable handler for managing the "Collect to DouList" dialog logic.
 * Can be used by any ViewModel that needs this functionality.
 */
class CollectionHandler(
    ...
    /**
     * Toggles the collection status for an item in a specific DouList.
     * @return The new overall 'isCollected' status for the item on success, or null on failure.
     */
    suspend fun toggleCollection(
        ...
        when (result) {
            is AppResult.Success -> {
                ...
                delay(300L) // Allow user to see the change
                dismissCollectDialog()
            }
