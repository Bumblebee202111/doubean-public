...
- Fix issue where the star group of a rating bar is distorted "randomly" when the rating is not multiple of 0.5 due to the built-in padding between by increasing step size to exactly this value (like Douban app), so that the stars is either accurate or biased downward
